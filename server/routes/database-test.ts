...
export async function getDatabaseInfo(req: Request, res: Response) {
  try {
    ...
    try {
      const [settingsCount] = await db.execute('SELECT COUNT(*) as total FROM lp_settings');
      const [settingsSample] = await db.execute('SELECT * FROM lp_settings LIMIT 5');
      ...
      settingsInfo = { error: 'Tabela lp_settings não existe ou não pode ser acessada' };
    }
