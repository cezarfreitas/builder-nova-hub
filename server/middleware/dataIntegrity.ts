...
export function cleanBrokenImageReferences() {
  try {
    ...
    if (heroData.background_image && !verifyImageExists(heroData.background_image)) {
      console.log(`⚠️  Imagem de fundo não encontrada: ${heroData.background_image}`);
...
export function generateDataStatusReport() {
  ...
  if (fs.existsSync(heroPath)) {
    ...
    try {
      ...
      report.hero.backgroundImageExists = verifyImageExists(heroData.background_image || '');
