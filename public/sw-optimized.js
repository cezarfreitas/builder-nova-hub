  ...
  event.waitUntil(
    Promise.all([
      ...
      caches.open(STATIC_CACHE).then((cache) => {
        ...
        return cache.addAll(
          CRITICAL_URLS.map((url) => new Request(url, { cache: "reload" })),
        );
      }),
...
self.addEventListener("error", (event) => {
  if (event.error && event.error.name === "QuotaExceededError") {
    console.warn(
      "🚨 Service Worker: Cache quota exceeded, cleaning old entries",
    );
...
async function cleanOldCacheEntries() {
  ...
  console.log(
    `🧹 Service Worker: Cleaned ${keysToDelete.length} old cache entries`,
  );
}
