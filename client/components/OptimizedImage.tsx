...
export function OptimizedImage({
  ...
  const generateOptimizedSrc = useCallback((baseSrc: string, targetWidth?: number) => {
...
export function useImageOptimization() {
  ...
  const resizeImage = useCallback((file: File, maxWidth: number, maxHeight: number): Promise<Blob> => {
    return new Promise((resolve, reject) => {
      const canvas = document.createElement('canvas');
      const ctx = canvas.getContext('2d');
      const img = new Image();
